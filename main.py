...
def parse_args():
    ...
    parser.add_argument("--architecture",type=str,default="hierarchical",choices=["hierarchical", "global", "gat", "gin"],help="model architecture",)
    ...
    if args.changer == 1:
        name = "second_Data_{}_Hidden_{}_Arch_{}_Pool_{}_WeightDecay_{}_Lr_{}.log".format(
        args.dataset,
        args.hidden_dim,
        args.architecture,
        args.pool_ratio,
...
def test(model: torch.nn.Module, loader, device, args, trial, e, if_test):
    ...
    for batch in loader:
        ...
        args.current_batch += 1
    #if args.save_hidden_output_test == False and args.save_hidden_output_test == True and if_test and (args.save_last_epoch_hidden_output == False or e == args.epochs-1):
...
def main(args, seed, save=True):
    ...
    #)
    test_acc, _ = test(model, test_loader, device, args, seed, e, 1)
    test_acc2, _ = test(model, test_loader2, device, args, seed, e, 1)
    if save == True:
        if args.changer == 1:
